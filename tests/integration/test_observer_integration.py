"""
Integration tests for the observer functionality in LiteAgent.

This module contains integration tests that use real API calls to test
observer functionality with actual agent interactions.
"""

import json
import pytest
from liteagent.observer import (
    AgentObserver, AgentEvent, AgentInitializedEvent, 
...
class TestObserverIntegration:
    """Integration tests for observer functionality with real agents."""
...
class TestTreeTraceObserverIntegration:
    """Integration tests for TreeTraceObserver functionality with real agents."""
    ...
    def test_tree_trace_observer_multi_agent(self, openai_agent, anthropic_agent):
        ...
        # Chat with both agents first to establish their contexts
        ...
        # For this test, we'll verify that the observer can track multiple independent agents
        # (The parent-child relationship would need to be established through the agent initialization
        # with explicit parent_context_id parameter, which is beyond this test's scope)
        ...
        # Verify both agents are tracked independently
        assert len(tree_observer.agents) >= 2
        assert agent1.agent_id in tree_observer.agents
        assert agent2.agent_id in tree_observer.agents
...
class TestAdvancedEventSerialization:
    """Test advanced event serialization scenarios."""
    
    def test_model_response_event_complex_objects(self):
        """Test ModelResponseEvent with complex object serialization."""
    ...
    def test_function_result_event_serialization(self):
        """Test FunctionResultEvent with various result types."""
...
class TestObserverInterfacePattern:
    """Test the observer interface pattern implementation."""
    
    def test_custom_observer_implementation(self):
        """Test creating a custom observer that implements the interface."""
